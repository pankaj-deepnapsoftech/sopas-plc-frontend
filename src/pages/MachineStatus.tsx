...
const MachineStatus: React.FC = () => {
  ...
  const fetchMachineData = useCallback(async (deviceId: string = 'PC-001') => {
    ...
    try {
      ...
      const url =
        actualDeviceId === 'all'
          ? plcApiUrl
          : `${plcApiUrl}?device_id=${encodeURIComponent(actualDeviceId)}`;
  ...
  // Transform PLC API array into the card/chart structure
  const transformMachineData = (rows: any[]) => {
    return rows.map((item: any) => {
      ...
      return {
        ...
        design: '-',
        count: item.count ?? 0,
        efficiency: item.percent ?? 0,
        error1: 0,
        error2: 0,
        status: 'ON', // PLC feed doesn’t provide status; assume ON
        duration: item.countdown != null ? `${item.countdown}s` : 'N/A',
        raw: item.raw || [],
      };
    });
  };
